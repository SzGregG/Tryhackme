...
**Persistance**
A method of maintaining persistance on Linux machines is through cron jobs. Cron jobs are schedualed taks on Linux. Cron job files are simple text files, so you can monitor any cahnges happeneing to them through auditd.  
Creation of new user accounts can be also tracked by user events with "useradd" for new users and "usermod" for user modification.
